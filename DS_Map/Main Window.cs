...
namespace DSPRE
   ...
   public partial class MainProgram : Form
      ...
      public bool alwaysBuilding0 { get; private set; } = false;
      ...
      private void updateBuildingListComboBox (bool interior)
         ...
         int nextVal = toolStripProgressBar.Value + 1;

         if (nextVal <= toolStripProgressBar.Maximum)
            toolStripProgressBar.Value = nextVal;
         }
      ...
      */
      ...
      private void mapScreenshotAllButton_Click (object sender, EventArgs e)
         {
         DialogResult d = MessageBox.Show (String.Format ("This will save {0} screenshots of every map available in the rom. " +
            "You may be notified about missing assets as the maps are prepared for saving. " +
            "This process may take a while depending on your machine. Do you wish to continue?", radio2D.Checked ? "2D" : "3D"),
            "Screenshot All Maps", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);

         if (d == DialogResult.No)
            return;

         string ScreenDirectory = RomInfo.gameVersion.ToString() + "_" + (radio2D.Checked ? "2D" : "3D");

         SetupMatrixEditor ();

         alwaysBuilding0 = true;

         for (int i = 0; i < selectMatrixComboBox.Items.Count; i++)
            {
            //currentMatrix = new GameMatrix (i);
            selectMatrixComboBox.SelectedIndex = i;

            for (int y = 0; y < currentMatrix.maps.GetLength (0); y++)
               {
               for (int x = 0; x < currentMatrix.maps.GetLength (1); x++)
                  {
                  /* Determine area data */
                  ushort headerID = 0;
                  if (currentMatrix.hasHeadersSection)
                     {
                     headerID = currentMatrix.headers[y, x];
                     }
                  else
                     {
                     List<string> result = HeaderSearch.AdvancedSearch (0, (ushort) internalNames.Count, internalNames, (int) MapHeader.SearchableFields.MatrixID, (int) HeaderSearch.NumOperators.Equal, selectMatrixComboBox.SelectedIndex.ToString ());
                  ...
                  if (currentMatrix.maps[y, x] == GameMatrix.EMPTY)
                     continue;
                  ...
                  AreaData areaData = new AreaData (h.areaDataID);
                  selectMapComboBox.SelectedIndex = currentMatrix.maps[y, x];
                  mapTextureComboBox.SelectedIndex = areaData.mapTileset + 1;
                  buildTextureComboBox.SelectedIndex = areaData.buildingsTileset + 1;
                  mainTabControl.SelectedTab = mapEditorTabPage;
                  ...
                  RenderMap (ref mapRenderer, ref buildingsRenderer, ref currentMapFile, ang, dist, elev, perspective, mapOpenGlControl.Width,
                     mapOpenGlControl.Height, mapTexturesOn, showBuildingTextures);
                  ...
                  newImage.Save (string.Format ("{0}/{1}.png", ScreenDirectory, currentMapFile.mapModel.models[0].Name));
                  }
               }
            }

         alwaysBuilding0 = false;

         MessageBox.Show (String.Format ("Screenshots saved in {0}/{1}.", Directory.GetCurrentDirectory(), ScreenDirectory),
      ...
      private void interiorRadioButton_CheckedChanged (object sender, EventArgs e)
         ...
         try
            ...
            if (currentMapFile.buildings.Count > 0)
